  ...
  const projects = [
    {
      ...
      icon: <Tv className="h-10 w-10 text-[#F68F30]" />,
      image: "/placeholder.svg?height=400&width=600",
      color: "bg-[#F68F30]",
      link: "https://dharmatelevision.org",
    },
    {
      ...
      icon: <Building className="h-10 w-10 text-[#9C282A]" />,
      image: "/placeholder.svg?height=400&width=600",
      color: "bg-[#9C282A]",
    ...
    },
    {
      ...
      icon: <Mountain className="h-10 w-10 text-[#1E3A5F]" />,
      image: "/placeholder.svg?height=400&width=600",
      color: "bg-[#1E3A5F]",
  ...
  return (
    <section className="py-20 md:py-32">
      <div className="container mx-auto px-4">
            ...
            >
              <div className={`order-2 ${index % 2 === 1 ? "lg:order-1" : "lg:order-2"}`}>
                <div className="relative aspect-video rounded-lg overflow-hidden shadow-xl">
                  ...
                  <div className={`absolute inset-0 ${project.color}/20 mix-blend-multiply`}></div>
                </div>
              </div>

              <div className={`order-1 ${index % 2 === 1 ? "lg:order-2" : "lg:order-1"}`}>
                <div className="flex items-center gap-4 mb-4">
                  <div className={`${project.color} p-3 rounded-lg text-white`}>{project.icon}</div>
                  <h3 className="font-playfair text-2xl md:text-3xl text-[#1E3A5F] dark:text-[#EBE175]">
                    {project.title}
                  </h3>
                </div>
                <p className="text-gray-700 dark:text-gray-300 mb-6">{project.description}</p>
                <Button asChild className={`${project.color} hover:${project.color}/90 text-white`}>
                  <a
                    href={project.link}
                    target="_blank"
        ...
        >
          <Button asChild size="lg" className="bg-[#1E3A5F] hover:bg-[#1E3A5F]/90 text-white">
